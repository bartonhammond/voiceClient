...
import 'package:MyFamilyVoice/tags/suggestions_textfield.dart';
...
class TagsState extends State<Tags> {
  ...
  double _widthCalc() {
    final int columns = widget.columns ?? 0;
    final int margin = widget.spacing.round();

    final int subtraction = columns * margin;
    final double width =
        (_width > 1) ? (_width - subtraction) / columns : _width;
...
/// Data List
class DataList extends ValueNotifier<bool> implements Item {
  DataList(
      {@required this.title,
      this.index,
